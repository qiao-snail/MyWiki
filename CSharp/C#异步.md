...
在计算机的上下文中，这意味着在另一个线程上执行一个进程或任务。一个线程是一系列的命令，一个代码块作为一个工作单元存在。操作系统可以管理多个线程，并在切换到另一个线程之前分配一个线程(“切片”)，然后切换到另一个线程来完成一些工作。在它的核心处理器可以简单地执行一个命令。它不需要同时做两件事情。操作系统通过将时间分配给不同的线程来模拟这一过程。
现在，如果你在混合中引入了多核处理器，那么事情就会同时发生。操作系统可以将时间分配给第一个处理器上的一个线程，然后在另一个处理器上分配相同的时间到另一个线程。
