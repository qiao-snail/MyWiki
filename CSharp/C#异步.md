# C#之异步
异步是相对于同步而言。跟多线程不能同一而论。如果以同步方式执行某个任务时，需要等待它完成，然后才能再继续执行另一个任务。而用异步执行某个任务时，可以在它完成之前执行另一个任务。异步最重要的体现就是不阻塞。

在计算机的上下文中，这意味着在另一个线程上执行一个进程或任务。一个线程是一系列的命令——一个代码块——作为一个工作单元存在。操作系统可以管理多个线程，并在切换到另一个线程之前分配一个线程(“切片”)，然后切换到另一个线程来完成一些工作。在它的核心(请原谅双关语)，处理器可以简单地执行一个命令——它不需要同时做两件事情。操作系统通过将时间分配给不同的线程来模拟这一过程。
现在，如果你在混合中引入了多个处理器/处理器，那么事情就会同时发生。操作系统可以将时间分配给第一个处理器上的一个线程，然后在另一个处理器上分配相同的时间到另一个线程。
...
在我看来，异步是一种编程的方式方法，模型。异步可以有新线程，也可以没有。取决于你的异步的实现方式。比如在CPU异步你用ThreadPool,Task.Run()等方法是创建了一个线程池的线程。在IO异步中，也可以实现创建新线程，但是IO操作，是硬件直接和内存交互，不需要CPU，所以创建新线程也没有意思。所以在IO异步中可以不创建线程，执行操作。那么在CPU执行中，异步有哪些好处呢。最直接的就是容易实现回调。
