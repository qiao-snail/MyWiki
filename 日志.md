...
如果需要绑定到Ioptions中，在依赖注入使用。可以使用Services.Config<T>(Action)(在Action中队对象赋值)
...
DotNetCore中使用HttpClient可以使用依赖注入使用
...
[difference between host and app configuration(应用配置和主机配置的区别)](https://github.com/dotnet/AspNetCore.Docs/issues/9278)
...
### Task.Run和Task.Factory的区别 ###

Task.Run是Task.Factory的一种默认实现
...
[官方](https://docs.microsoft.com/zh-cn/dotnet/standard/threading/overview-of-synchronization-primitives)

* 数据仓储是对数据管理的对象
* 工作单元测试提供了跨越多个数据仓储时，使用同一个数据上下文(在同一个连接中完成，事务操作)
* ABP中如果在一个仓储中对另一个仓储操作，那么该两个不同的操作使用的时同一个连接。


* EFCore可以数据库和对象分别定义，再通过Attribute来关联。--- 在代码中定义好对象，并将对象和数据库中的表和字段一一对应。
  1. `Table("TableName")`特性对应表明
  2. `Column("propertyName",Properties:[Order = int],[TypeName = string])` 分别对应字段名和排序，及字段类型
  3. datetype2对应的数据类型是long

* mysql主键使用uuid的长度为binary-16

---

2021-05-13

Dictionary To Json,转换的json字符串为：
"dictionaryName":{
  "key1":"value1",
  "key2":"value2",
  "key3":"value3",
...
Abp VNext 本身是支持属性注入的，但是需要注意的是，所有的流程都需要依赖注入来实现，自己new的对象是不会实现注入的
